...
(def beet-1 '((0 55 750 1 64) (750 55 250 1 64) (1000 55 2000 1 64) (3000 60 750 1 64)
              (3750 59 250 1 64) (4000 60 750 1 64) (4750 62 250 1 64)))

(def beet-2 '((0 77 500 1 64) (500 82 1500 1 64) (2000 81 250 1 64) (2250 82 250 1 64)
              (2500 84 500 1 64) (3000 82 500 1 64)))

(def bocc-1 '((0 81 250 1 64) (250 80 250 1 64) (500 81 250 1 64) (750 83 250 1 64)
              (1000 81 500 1 64)))

(def c-1 '((0 58 2000 1 64) (2000 63 1000 1 64) (3000 62 1000 1 64) (4000 63 1000 1 64)
           (5000 66 1000 1 64) (6000 70 1000 1 64) (7000 70 3000 1 64) (10000 75 1000 1 64)))

(def s-1 '((0 60 1000 1 64) (1000 65 2500 1 64) (3500 64 500 1 64) (4000 65 500 1 64)
...
(defn count-xs
  ...
  [list]
  (reduce (fn [n item] (if (= :x item) (inc n) n)) 0 list))
...
(defn find-patterns [target-pattern database-works database-names]
  (if (empty? database-works)
    ()
    (if-let [test (pattern-match target-pattern (get-intervals (first database-works)))]
      (cons (list test (first database-names)) (find-patterns target-pattern (rest database-works) (rest database-names)))
      (find-patterns target-pattern (rest database-works) (rest database-names)))))

(defn sources [target list-of-sources patterns]
  (reduce (fn [hits [pattern source counter]]
            (let [timing (get-timing target patterns)
                  thing (if timing
                          (change-channel (add-timing timing source) counter)
                          [])]
              (cons thing hits)))
          (map vector patterns list-of-sources (range 0))))
...
(defn source-it
  ...
  (let [patterns (find-the-patterns target list-of-sources database-names)
        results (sources target list-of-sources patterns)
